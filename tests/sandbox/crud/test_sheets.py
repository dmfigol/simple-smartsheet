...
class TestCreateSheet:
    ...
    def setup_teardown(self, custom_vcr, smartsheet, new_sheet):
        yield
        with custom_vcr.use_cassette("test_sheets/fixtures/create_sheet_teardown.yaml"):
            sheets = smartsheet.sheets.list()
            for sheet in sheets:
...
class TestDeleteSheet:
    ...
    def setup_teardown(self, custom_vcr, smartsheet, sheet_to_delete):
        with custom_vcr.use_cassette("test_sheets/fixtures/delete_sheet_setup.yaml"):
            sheets = smartsheet.sheets.list()
    ...
    @pytest.mark.asyncio
    @pytest.mark.vcr
    async def test_delete_sheet_async(self, async_smartsheet, sheet_to_delete):
...
class TestUpdateSheet:
    ...
    def setup_teardown(
        ...
        with custom_vcr.use_cassette("test_sheets/fixtures/update_sheet_setup.yaml"):
            new_sheet = create_sheet_with_rows(smartsheet, sheet_to_update, rows_data)
        yield
        with custom_vcr.use_cassette("test_sheets/fixtures/update_sheet_teardown.yaml"):
    ...
    @staticmethod
    def _get_delete_rows_ids(sheet):
        sheet.build_index([{"columns": ("Email address",), "unique": True}])
        row1 = sheet.get_row(filter={"Email address": "alice.smith@globex.com"})
        row2 = sheet.get_row(filter={"Email address": "bob.lee@acme.com"})
    ...
    @staticmethod
    def _check_delete_rows(sheet, row_ids):
        assert len(sheet.rows) == 1
        for row_id in row_ids:
            assert sheet.get_row(row_id=row_id) is None
    ...
    def test_sheet_delete_rows(
        ...
        sheet = smartsheet.sheets.get(name=sheet_to_update.name)
        row1_id, row2_id = self._get_delete_rows_ids(sheet)

        result = smartsheet.sheets.delete_rows(sheet.id, [row1_id, row2_id])
        assert result.message == "SUCCESS"

        updated_sheet = smartsheet.sheets.get(id=sheet.id)
        self._check_delete_rows(updated_sheet, [row1_id, row2_id])
    ...
    async def test_sheet_delete_rows_async(
        ...
        result = await async_smartsheet.sheets.delete_rows(sheet.id, [row1_id, row2_id])
        assert result.message == "SUCCESS"
    ...
    @staticmethod
    def _gen_many_rows_to_delete(sheet: Sheet) -> List[Row]:
        # happens on >=500 rows
        return [
            Row(to_bottom=True, cells=sheet.make_cells({"Full Name": f"Test {i}"}))
            for i in range(500)
        ]

    @pytest.mark.vcr
    def test_sheet_delete_many_rows(
        self, smartsheet: Smartsheet, sheet_to_update: Sheet
    ) -> None:
        sheet = smartsheet.sheets.get(name=sheet_to_update.name)
    ...
    @pytest.mark.asyncio
    @pytest.mark.vcr
    async def test_sheet_delete_many_rows_async(
        self, async_smartsheet: AsyncSmartsheet, sheet_to_update: Sheet
    ) -> None:
        sheet = await async_smartsheet.sheets.get(name=sheet_to_update.name)
